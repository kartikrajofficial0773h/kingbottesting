# KANGING KANGING
# DONT KANG IF KANG GIVE CREDITS 
...
async def gben(userbot):
    ...
    await dark.edit(f"Trying to ban you globally ban in process...")
    ...
    try:
        ...
        pass
    return await dark.edit(
        f"**This user is Globally Tapped [{user.first_name}](tg://user?id={user.id}) Affected Chats😏 : {a} **"
    )
...
async def gunben(userbot):
    ...
    me = await dc.client.get_me()
    if not sender.id == me.id:
        dark = await dc.reply("`Wait Let Me ungban this gay😂`")
    else:
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await dark.edit(f"**Ungbaning this gay.. AFFECTED CHATS - {a} **")
            except:
                b += 1
    else:
    ...
    try:
        ...
        pass
    return await dark.edit(
        f"**Ungbanned sucessfully!! I gave u one more chance! ; USER - [{user.first_name}](tg://user?id={user.id}) CHATS : {a} **"
    )
